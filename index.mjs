...
const DOWNLOAD_IMAGE = process.env.DOWNLOAD_IMAGE || getInput('DOWNLOAD_IMAGE');
...
async function generateBirthdayImage(employees, slackUsers) {
    ...
    parts.push({
        ...
        Also place the logo (from "Logo image", first image following this text) unobtrusively, without overdoing it, for example on
        ...
        Draw the birthday employees with the correct gender and based on their images.
...
/** Download image to local FS (debugging helper). */
async function downloadImageToFilesystem(fileBuffer, filename) {
...
async function main() {
    try {
        ...
        if (channels.length) {
            ...
            if (DOWNLOAD_IMAGE) {
                await downloadImageToFilesystem(imageBuffer, filename);
            } else {
